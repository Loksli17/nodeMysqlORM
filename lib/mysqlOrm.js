...
modelMysql.prototype.save = async function(_data){
    ...
    if(validation != true){
        for(let key in validation){
            if(this.validateMessege){
                ...
                for(let fieldKey in this.fields){
                    if(fieldKey == key){
                        errors.push({field: key, message: `${this.validateMessege}. ${this.fields[key]}: ${validation[key]}`});4
                        flag = true;
                    }
                }
                if(!flag) errors.push({field: key, message: `${this.validateMessege}. ${key}: ${validation[key]}`});
            }else{
                errors.push({field: key, message: `Model wasn't validated. Error in field: ${keyValidation}. ${validation[key]}`});
            }
        ...
        //             errors.push({field: key, message: this.validateMessege + '. ' + this.fields[keyValidation] + ': ' + validation[keyValidation]});
        //         }
        //     }
        //     errors.push({field: key, message: `Model wasn't validated. Error in field: ${keyValidation}. ${validation[keyValidation]}`});
        // }
        // errors.push({field: key, message: `Model wasn't validated. Error in field: ${keyValidation}. ${validation[keyValidation]}`});
    }

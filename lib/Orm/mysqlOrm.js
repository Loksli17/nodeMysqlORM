...
modelMysql.prototype.find = async function(method, conditions){
    ...
    //NEW WHEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEE THEREEEEEEEEEEEEEE
    if(conditions.newWhere != undefined){

        conditions.newWhere[0] = this.escape(conditions.newWhere[0]);
        //checking of where data
        for(let key in conditions.newWhere[1]){
            switch(typeof conditions.newWhere[1][key]){
                case 'string':
                    conditions.newWhere[1][key] = this.escape(conditions.newWhere[1][key]);
                    conditions.newWhere[1][key] = conditions.newWhere[1][key].replace(/$/g, '');
                    break;
                case 'number':
                    conditions.newWhere[1][key] = this.escape(conditions.newWhere[1][key]);
                    break;
                case 'boolean':
                    break;
                case 'object':
                    if(!Array.isArray(conditions.newWhere[1][key])){
                        throw 'Error: type of where option must be string, number, array with operator IN, boolean or JavaScript date';
                    }
                    if(conditions.newWhere[0].indexOf('IN') == -1 && conditions.newWhere[0].indexOf('in') == -1)
                        throw 'Error: you can use array only with operatop `IN`';
                    break;
                default:
                    throw 'Error: type of where option must be string, number, boolean or JavaScript date';
                    break;
            }

            // console.log(conditions.newWhere[0].indexOf("$" + key), "$" + key, conditions.newWhere[0], typeof conditions.newWhere[0]);
            if(typeof conditions.newWhere[0] != 'string' || conditions.newWhere[0].indexOf("$" + key) == -1){
                throw 'Error: first element of WHERE must be a string, and keys from second element must be match with :values form first.';
            }

            if(conditions.newWhere[0].indexOf('IN') != -1 || conditions.newWhere[0].indexOf('in') != -1){
                conditions.newWhere[0] = conditions.newWhere[0].replace('$' + key, conditions.newWhere[1][key]);
            }else{
                conditions.newWhere[0] = conditions.newWhere[0].replace('$' + key, "'" + conditions.newWhere[1][key] + "'");
            }
        ...
        for(let i = 0; i < conditions.newWhere[0].length; i++){
            if(conditions.newWhere[0][i] == '$'){
                throw 'Error: first element of WHERE must be a string, and keys from second element must be match with :values form first.';
            }
        }

        conditionsQuery += ' WHERE ' + conditions.newWhere[0];
    }

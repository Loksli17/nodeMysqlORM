...
modelMysql.prototype.checkTypeRules = function(){
    let
        permissionList = ['integer', 'json', 'boolean', 'string', 'date', 'email', 'phone'],
        flFirst        = false,
        flSecond       = false,
        keyFirst       = '',
        keySecond      = '';
    ...
    for(let keyFirstRule in this.rules){
        ...
        for(let i = 0; i < permissionList.length; i++){
            if(permissionList[i] == keyFirstRule) flFirst = true; keyFirst;
        }
        if(!flFirst) continue;
        ...
        for(let keySecondRule in this.rules){
            flSecond = false;
            for(let i = 0; i < permissionList.length; i++){
                if(permissionList[i] == keySecondRule && keySecondRule != keyFirstRule) flSecond = true;
            }
